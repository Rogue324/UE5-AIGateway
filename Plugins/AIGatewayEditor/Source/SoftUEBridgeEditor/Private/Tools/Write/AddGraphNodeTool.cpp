...
namespace
{
	...
	bool BindVariableNode(UK2Node* K2Node, UBlueprint* Blueprint, const TSharedPtr<FJsonObject>& Properties, FString& OutError)
	{
		...
		if (UK2Node_VariableGet* GetNode = Cast<UK2Node_VariableGet>(K2Node))
		{
			...
			{
				OutError = FString::Printf(TEXT("Variable '%s' was not found on Blueprint '%s'. Create it first with add-blueprint-variable, then add the variable node with add-blueprint-k2-node or add-graph-node."), *VariableName, *Blueprint->GetName());
				return false;
			}
		}
...
FString UAddGraphNodeTool::GetToolDescription() const
{
	return TEXT("Add a node to a Blueprint or Material graph by class name with intelligent auto-positioning. "
		...
		"Prefer add-blueprint-k2-node for Blueprint function calls and Blueprint variable get/set nodes because it performs stronger binding validation. "
	...
	FString& OutError)
{
	...
	if (NodeClass->IsChildOf<UK2Node_AddComponent>())
	{
		OutError = TEXT("K2Node_AddComponent is not supported through add-graph-node because it can assert during pin allocation. Do not create component-adding Blueprint nodes through this tool. Use add-component for level actors, or use a dedicated Blueprint component workflow instead.");
		return nullptr;
	}
